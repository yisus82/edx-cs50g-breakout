...
--[[
  ...
  @param {number} skin - the skin for our paddle
]]
...
--[[
  Handles logic for paddle movement, ensuring we can't move outside of
...
function Paddle:update(dt)
  ...
  -- math.max here ensures that we're the greater than 0 or the player's
  -- current calculated Y position when pressing up so that we don't
  -- go into the negatives; the movement calculation is simply our
  -- previously-defined paddle speed scaled by dt
  if self.dx < 0 then
    self.x = math.max(0, self.x + self.dx * dt)
    -- we use math.min to ensure we don't go any farther than the bottom
    -- of the screen minus the paddle's height (or else it will go partially
...
function Paddle:render()
  love.graphics.draw(gImages['main'], gQuads['paddles'][self.size + 4 * (self.skin - 1)],
    self.x, self.y)
end
